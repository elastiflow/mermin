...
impl TestHarness {
    ...
    pub async fn receive_event(&mut self) -> Result<ParsedHeader> {
        debug!("Waiting for event with timeout of {:?}", self.config.receive_timeout);
...
pub async fn setup_test_with_config(config: TestConfig) -> Result<TestHarness> {
    ...
    program.load()?;
    let _link = program.attach(&config.interface, config.tc_attach_type)
        .context(format!("Failed to attach program to interface '{}'", config.interface))?;
    
    info!("Successfully attached program to interface '{}'", config.interface);
...
macro_rules! define_header_test {
    ...
    ($test_name:ident, $header_type:ty, $packet_type:expr, $setup_fn:expr, $verify_fn:expr) => {
        define_header_test!($test_name, $header_type, $packet_type, $setup_fn, $verify_fn, {
    ...
    ($test_name:ident, $header_type:ty, $packet_type:expr, $setup_fn:expr, $verify_fn:expr, $config_expr:expr) => {
        ...
        async fn $test_name() -> Result<(), anyhow::Error> {
            ...
            let client = UdpSocket::bind(config.client_bind_addr)
                .context(format!("Failed to bind client socket to {}", config.client_bind_addr))?;
                
            debug!("Connecting client socket to {}", config.server_addr);
            client.connect(config.server_addr)
                .context(format!("Failed to connect client socket to {}", config.server_addr))?;
