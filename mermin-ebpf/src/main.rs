...
use aya_ebpf::{bindings::TC_ACT_PIPE, macros::classifier, programs::TcContext, EbpfContext};
...
fn parse_ethernet_header(ctx: &TcContext, parser: &mut Parser) -> Result<(), ()> {
    ...
    match eth_hdr.ether_type {
        ...
        _ => {
            debug!(
                ctx,
                "ethernet header contains unsupported ether type: {}", eth_hdr.ether_type as u16
            );
    ...
    match next_hdr {
        ...
        | IpProto::Ipip => {
            ...
            if parser.outer_src_ip_addr.is_none() {
                parser.outer_src_ip_addr =
                    Some(CReprIpAddr::new_v4(u32::from_be_bytes(ipv4_hdr.src_addr)));
            }
            if parser.outer_dst_ip_addr.is_none() {
                parser.outer_dst_ip_addr =
                    Some(CReprIpAddr::new_v4(u32::from_be_bytes(ipv4_hdr.dst_addr)));
            }
