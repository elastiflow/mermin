    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            while let Some(bytes) = self.ring_buf.next() {
                ...
                trace!(
                    event.name = "packet.received",
                    packet.source = "ring_buffer",
                    "read packet metadata from ring buffer"
                );
                ...
                match self.router.should_process(&packet_meta) {
                    Ok(true) => {
                        if let Err(e) = self.packet_meta_tx.send(packet_meta).await {
                            warn!(
                                event.name = "packet.send_failed",
                                channel = "k8s_attribution",
                                error.message = %e,
                                "failed to send packet to k8s attribution channel"
                            );
                        }
                    }
                    Ok(false) => {
                        debug!(
                            ...
                            "packet metadata did not match any filters; skipping"
                        );
                    }
                    Err(e) => {
                        warn!(
                            event.name = "packet.routing_failed",
                            error.message = %e,
                            "failed to route packet meta; skipping"
                        );
                    }
                }
            }
