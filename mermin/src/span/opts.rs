...
/// span_generic: 30 - If no activity has been observed for a flow span in the last 30 seconds, generate a record. unless...
/// icmp: 10 - If no activity has been observed for an ICMP flow span in the last 10 seconds, generate a record.
/// tcp: 20 - If no activity has been observed for a TCP flow span in the last 20 seconds, generate a record. unless...
/// tcp-fin: 5 - If we see a FIN flag for a TCP flow span, generate a record 5 secs after the flag.
/// tcp-rst: 5 - If we see an RST flag for a TCP flow span, generate a record 5 secs after the flag.
