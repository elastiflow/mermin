...
impl PacketWorker {
    ...
    pub async fn run(mut self) {
        while let Some(packet) = self.packet_meta_rx.recv().await {
            let _ = self.upsert_packet_meta(packet);

            // TODO: REMOVE this is just for testing sending all flows to the exporter
            // let _ = self.flow_span_tx.send(span).await;
        }
    }

    fn upsert_packet_meta(&self, packet: PacketMeta) -> Result<(), Error> {
