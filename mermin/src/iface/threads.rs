...
pub fn spawn_controller_thread(
    ...
    ebpf_return: oneshot::Sender<Ebpf>,
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // Return the Ebpf object so it can be reused across pipeline restarts.
            // This keeps eBPF programs loaded in the kernel even while the pipeline
            // is restarting. The send happens after all other work is done so the
            // receiver (Pipeline::preserve_and_shutdown) is guaranteed to find it
            // ready after manager.shutdown() joins this thread.
            let _ = ebpf_return.send(controller.into_ebpf());
        })
}
