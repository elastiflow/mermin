...
- `transport` - Filter by transport protocol
...
- `interface_name` - filter by network interface name
...
- `interface_index` - filter by network interface index  
...
- `interface_mac` - filter by network interface MAC address  
...
- `connection_state` - filter by TCP connection state
...
- `ip_dscp_name` - filter flows based on the DSCP ([Differentiated Services Code Point](https://en.wikipedia.org/wiki/Differentiated_services#Configuration_guidelines)) names  
  **Supported values in [patterns](#matchnot_match-patterns):** Any valid interface name (supports [globs](https://docs.rs/globset/latest/globset/#syntax))  
  **Examples:**  
...
- `ip_ecn_name` - filter flows based on ECN ([Explicit Congestion Notification](https://en.wikipedia.org/wiki/Explicit_congestion_notification)) values  
...
- `ip_ttl` - filter flows based on the IP TTL ([Time To Live](https://en.wikipedia.org/wiki/Time_to_live)) values  
...
- `icmp_type_name` - filter flows based on [ICMP type](https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml) names (converted to a [snake case](https://en.wikipedia.org/wiki/Snake_case))  
...
- `icmp_code` - filter flows based on [ICMP codes](https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml)  
