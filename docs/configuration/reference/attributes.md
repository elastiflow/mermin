...
*   `metadata` attribute - list of strings, default `["[*].metadata.namespace", "[*].metadata.name", "[*].metadata.uid"]`.
    ...
    **Example:** Extract all resource names but namespace only for the Pods
    ...
    ```
*   `label` block - [metadata extraction object](attributes.md#metadata-extraction-object), `{}` (no labels are extracted by default).

    The label block configures how to extract Kubernetes labels to Otel attributes, can be defined multiple times to extract multiple labels. _Not implemented_
    ...
    **Example:** Extract all Service labels with `kubernetes.io/` prefix to Otel attribute named after label suffix without any value modifications
    ...
    ```
*   `annotation` block - [metadata extraction object](attributes.md#metadata-extraction-object), `{}` (no annotations are extracted by default).

    The label block configures how to extract Kubernetes annotations to Otel attributes, can be defined multiple times to extract multiple labels. _Not implemented_
...
*   `sources` attribute - list of [association objects](attributes.md#association-object), please see the [default configuration](https://github.com/elastiflow/mermin/blob/beta/docs/configuration/default/config.hcl) for the default for each Kubernetes kind.
