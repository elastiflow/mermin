...
Mermin can parse and track traffic through common tunnel encapsulation protocols. The parser configuration allows you to specify which ports these tunnel protocols use, enabling Mermin to properly identify and process encapsulated traffic.
...
```hcl
# Parser configuration for tunnel protocol detection
parser {
  # Geneve tunnel detection (used by Cilium, NSX-T)
  geneve_port = 6081  # IANA standard

  # VXLAN tunnel detection (used by Flannel, Calico)
  vxlan_port = 4789   # IANA standard

  # WireGuard tunnel detection (encrypted overlay)
  wireguard_port = 51820  # Default WireGuard port
}
```
