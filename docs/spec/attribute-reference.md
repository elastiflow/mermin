...
This document is a reference for all of the available attributes in mermin. It describes the attributes that may appear on flow spans and related telemetry, along with their types and requirement levels.
...
* [Attribute Tables](#attribute-tables)
  ...
  * [Tunnel & Ip-in-Ip & IPSec Attributes](#tunnel-ip-in-ip-ipsec-attributes)
  * [Kubernetes Attributes](#kubernetes-attributes)
  * [Network Policy Attributes](#network-policy-attributes)
  * [Process & Container Attributes](#process-container-attributes)
...
| `flow.direction`        | `string`  | The inferred direction of the flow from the observer's perspective.                       | One of: `forward`, `reverse`, or `unknown`. Mirrors IPFIX biflow concepts. See [Flow Direction](#flow-direction) for details.     |          | ✓          |
...
> **Note:** These attributes use `source.k8s.*` / `destination.k8s.*` prefixes rather than standard OTel `k8s.*` attributes. See [Why `source.k8s.*` Instead of `k8s.source.*`?](#why-sourcek8s-instead-of-k8ssource) for the rationale.
