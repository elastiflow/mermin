...
| `client.address`              | `string`   | Client IP address or hostname.                                                          | Hostname if resolution enabled, otherwise IP. Complements source/dest. | ✓        | ? CLIENT/SERVER span |
| `client.port`                 | `long`     | Client port number.                                                                     | Complements source/dest port.                                          | ✓        | ? CLIENT/SERVER span |
| `server.address`              | `string`   | Server IP address or hostname.                                                          | Hostname if resolution enabled, otherwise IP. Complements source/dest. | ✓        | ? CLIENT/SERVER span |
| `server.port`                 | `long`     | Server port number.                                                                     | Complements source/dest port.                                          | ✓        | ? CLIENT/SERVER span |
...
| `process.executable.name`                       | `string`   | The name of the binary associated with the socket for this flow (from comm field).    | Captured via LSM socket hooks (kernel 5.7+). | ✓        | ~ |
| `process.pid`                                   | `long`     | The PID of the process associated with the socket for this flow.    | Captured via LSM socket hooks (kernel 5.7+). | ✓        | ~ |
| `process.tgid`                                  | `long`     | The TGID (thread group ID/main process ID) associated with the socket for this flow. | Captured via LSM socket hooks (kernel 5.7+). | ✓        | ~ |
| `process.cgroup.id`                             | `long`     | The cgroup ID for container/namespace correlation.                  | Captured via LSM socket hooks (kernel 5.7+). | ✓        | ~ |
| `process.uid`                                   | `long`     | The user ID (UID) of the process owning the socket.                 | Captured via LSM socket hooks (kernel 5.7+). | ✓        | ~ |
