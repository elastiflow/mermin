...
```sh
fmtconvert -from hcl -to yaml docs/deployment/examples/local/config.hcl > docs/deployment/examples/local/config.yaml
```
...
```shell
cargo run --release --config 'target."cfg(all())".runner="sudo -E"' -- --config <(fmtconvert -from hcl -to yaml docs/deployment/examples/local/config.hcl)
```
